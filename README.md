...
Для сборки требуется установленный Docker.

```bash
make
```

Эта команда запускает универсальную сборку в Docker и просто компилирует все `*.tex` файлы в PDF.
Проверки, специфичные для этого репозитория, например lint LaTeX-исходников и проверка числа страниц итоговых PDF, выполняются только при релизной сборке:

```bash
make release_docker
```

Проверить LaTeX-исходники линтером можно так:

```bash
make lint
```

Автоформатирование `.tex` файлов запускается вручную:

```bash
make format
```

Проверить, предложил бы форматтер изменения, можно без перезаписи файлов:
...
Автоматически запустится процесс GitHub Actions, который выполнит строгую релизную сборку `make release` внутри контейнера и закончится примерно через 2-3 минуты. Его можно наблюдать во вкладке *Actions* в репозитории.
...
Для автоматической компиляции библиографии используется специальный файл **.bib** (`biblio/bibliography.bib`), в который необходимо добавлять свои источники в нужном формате.

Рекомендую удалить все содержимое файла **bibliography.bib** и добавлять по одному свои источники, так как при случайном добавлении дубликата bibtex будет выдавать ошибки.
...
3. Полученное содержимое вставляем в файл **biblio/bibliography.bib**, ***заменяя автоматически сгенерированное название на DOI статьи*** (это распространенная практика в научной сфере, так вы не добавите случайно дубликат в вашу работу под разными именами).
