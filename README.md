...
1. Ошибки, генерируемые bibtex, игнорируются, а сборка не прерывается! Если файл собрался неправильно, надо открывать логи (файл.log), все ошибки сохранены там.

2. Странные, непонятно откуда взявшиеся символы могут появиться перед списком литературы. Это может случится, если картинка будет последней в главе, а за ней не будет никакого текста. Пофиксить это скорее всего не представляется возможным, стоит этого просто избегать.
