    ...
    stages {
        stage('BuildThesis') {
            steps {
                ...
                sh 'docker run --rm -i -v ${PWD}:/:Z texlive/texlive bash -c "ls && pdflatex -interaction=nonstopmode presentation.tex && pdflatex -interaction=nonstopmode presentation.tex"'
                sh 'make clean'
                sh 'mkdir -p Thesis'
